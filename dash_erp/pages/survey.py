import pandas as pd
from dash import (html, Input, Output, dcc, dash_table, callback, register_page)
...
layout=dbc.Container([dbc.Row([dbc.Col([html.P('Survey Results 2021', className='title'), html.P('Redesign Data App With Dash Plotly', className='subtitle'),   dbc.Col(html.Img(src=image_path, className='img')
)])]),
                          dbc.Row([
...
@callback(Output('graph1', 'figure'), Output('graph2', 'figure'), Output('table', 'data'), Input('age', 'value'), Input('department', 'value'))
